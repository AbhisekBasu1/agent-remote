/*
See LICENSE-APPLE.txt for the Apple NullAudio sample license and the repository
LICENSE for the Agent Remote modifications.

Abstract:
A virtual loopback device that publishes decoded DualSense Bluetooth
microphone samples to Core Audio applications.
*/

/*==================================================================================================
	DualSenseBridgeMic.c
==================================================================================================*/
...
//==================================================================================================
#pragma mark -
#pragma mark DualSense Bridge Mic State
//==================================================================================================
...
#define										kPlugIn_BundleID				"io.github.abhisekbasu1.AgentRemote.MicrophoneDriver"
static pthread_mutex_t						gPlugIn_StateMutex				= PTHREAD_MUTEX_INITIALIZER;
static UInt32								gPlugIn_RefCount				= 0;
static AudioServerPlugInHostRef				gPlugIn_Host					= NULL;
static const AudioObjectPropertySelector	kPlugIn_CustomPropertyID		= 'PCst';

#define										kBox_UID						"DualSenseBridgeMicBox_UID"
static CFStringRef							gBox_Name						= NULL;
static Boolean								gBox_Acquired					= true;

#define										kDevice_UID						"DualSenseBridgeMic_UID"
#define										kDevice_ModelUID				"DualSenseBridgeMic_ModelUID"
static pthread_mutex_t						gDevice_IOMutex					= PTHREAD_MUTEX_INITIALIZER;
static Float64								gDevice_SampleRate				= 48000.0;
static UInt64								gDevice_IOIsRunning				= 0;
#define										kDevice_RingBufferSize			16384U
static Float64								gDevice_HostTicksPerFrame		= 0.0;
static UInt64								gDevice_NumberTimeStamps		= 0;
static Float64								gDevice_AnchorSampleTime		= 0.0;
static UInt64								gDevice_AnchorHostTime			= 0;

// Audio written by DualSense Bridge is consumed by Codex (or another recording
// app) from the input side. The FIFO retains the newest audio on overrun and
// produces silence on underrun.
static Float32								gLoopbackRing[kDevice_RingBufferSize * 2];
static UInt32								gLoopbackReadFrame				= 0;
static UInt32								gLoopbackWriteFrame				= 0;
static UInt32								gLoopbackAvailableFrames		= 0;
...
static OSStatus	NullAudio_Initialize(AudioServerPlugInDriverRef inDriver, AudioServerPlugInHostRef inHost)
{
	...
	if(gBox_Name == NULL)
	{
		gBox_Name = CFSTR("DualSense Bridge Mic");
	}
...
static OSStatus	NullAudio_GetPlugInPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyManufacturer:
			...
			*((CFStringRef*)outData) = CFSTR("DualSense Bridge");
			*outDataSize = sizeof(CFStringRef);
			break;
...
static OSStatus	NullAudio_GetBoxPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyModelName:
			//	This is the human readable name of the maker of the box.
			FailWithAction(inDataSize < sizeof(CFStringRef), theAnswer = kAudioHardwareBadPropertySizeError, Done, "NullAudio_GetBoxPropertyData: not enough space for the return value of kAudioObjectPropertyManufacturer for the box");
			*((CFStringRef*)outData) = CFSTR("DualSense Bridge Virtual Microphone");
			*outDataSize = sizeof(CFStringRef);
			break;
...
static OSStatus	NullAudio_GetDevicePropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyName:
			...
			*((CFStringRef*)outData) = CFSTR("DualSense Bridge Mic");
			*outDataSize = sizeof(CFStringRef);
			break;
...
static OSStatus	NullAudio_GetStreamPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyName:
			...
			*((CFStringRef*)outData) = (inObjectID == kObjectID_Stream_Input) ? CFSTR("DualSense Mic Input") : CFSTR("DualSense Mic Feed");
			*outDataSize = sizeof(CFStringRef);
			break;
...
static OSStatus	NullAudio_StartIO(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID)
{
	...
	else if(gDevice_IOIsRunning == 0)
	{
		...
		gLoopbackReadFrame = 0;
		gLoopbackWriteFrame = 0;
		gLoopbackAvailableFrames = 0;
...
static OSStatus	NullAudio_GetZeroTimeStamp(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, Float64* outSampleTime, UInt64* outHostTime, UInt64* outSeed)
{
	...
	//	Calculate the next host time. This device can stay open while no client
	//	is actively pulling audio (the app prewarms its output queue). When IO
	//	resumes much later, advancing only one ring period per call leaves the
	//	reported clock minutes or hours behind wall time. Core Audio then races
	//	through callbacks to catch up, draining the speech FIFO several times
	//	faster than real time. Jump directly to the current completed period so
	//	the virtual device clock can never accumulate that stale-time debt.
	...
	//	Go to the newest completed zero-timestamp period if the clock was idle.
	if(theNextHostTime <= theCurrentHostTime)
	{
		gDevice_NumberTimeStamps = (UInt64)(
			((Float64)(theCurrentHostTime - gDevice_AnchorHostTime)) /
...
static OSStatus	NullAudio_DoIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, AudioObjectID inStreamObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo, void* ioMainBuffer, void* ioSecondaryBuffer)
{
	//	The output stream is the app-facing feed and the input stream is the
	//	virtual microphone consumed by recording applications. Keep a FIFO
	//	between them so the producer and consumer may run in different HAL IO
	//	cycles. Both streams use interleaved stereo Float32 at 48 kHz.
	   ...
	   (inStreamObjectID == kObjectID_Stream_Output))
	{
		...
		{
			if(gLoopbackAvailableFrames == kDevice_RingBufferSize)
			{
				gLoopbackReadFrame = (gLoopbackReadFrame + 1) % kDevice_RingBufferSize;
				--gLoopbackAvailableFrames;
			}
			gLoopbackRing[gLoopbackWriteFrame * 2] = source[frame * 2];
			gLoopbackRing[gLoopbackWriteFrame * 2 + 1] = source[frame * 2 + 1];
			gLoopbackWriteFrame = (gLoopbackWriteFrame + 1) % kDevice_RingBufferSize;
			++gLoopbackAvailableFrames;
		}
		pthread_mutex_unlock(&gDevice_IOMutex);
	}
	else if((inOperationID == kAudioServerPlugInIOOperationReadInput) &&
	        (inStreamObjectID == kObjectID_Stream_Input))
	{
		Float32* destination = (Float32*)ioMainBuffer;
		pthread_mutex_lock(&gDevice_IOMutex);
		for(UInt32 frame = 0; frame < inIOBufferFrameSize; ++frame)
		{
			if(gLoopbackAvailableFrames > 0)
			{
				destination[frame * 2] = gLoopbackRing[gLoopbackReadFrame * 2];
				destination[frame * 2 + 1] = gLoopbackRing[gLoopbackReadFrame * 2 + 1];
				gLoopbackReadFrame = (gLoopbackReadFrame + 1) % kDevice_RingBufferSize;
				--gLoopbackAvailableFrames;
			}
			else
			{
				destination[frame * 2] = 0.0f;
				destination[frame * 2 + 1] = 0.0f;
			}
		}
		pthread_mutex_unlock(&gDevice_IOMutex);
	}
